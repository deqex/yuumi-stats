export function genScore(player, allParticipants, gameDuration, queueId) {
    ...
    const kdaScore      = (((p.kills || 0) * 1.4 + (p.assists || 0) - (p.deaths || 0)) / dur) * 1200;
    const visionScore   = (p.visionScore || 0) * 400 / dur;
    const csScore       = Math.round((((p.totalMinionsKilled || 0) + (p.totalEnemyJungleMinionsKilled || 0) + (p.totalAllyJungleMinionsKilled || 0)) / dur) * 60);
    const enchanterScore = (((p.totalDamageShieldedOnTeammates || 0) + (p.totalHealsOnTeammates || 0)) * 1.3) / dur;
    const tankScore     = (((p.damageSelfMitigated || 0)) / dur)*0.5;
    let ccScore         = (p.timeCCingOthers || 0) * 300 / dur;
    const goldScore     = totalGold > 0 ? ((((p.goldEarned || 0) / totalGold) * 100) - 5) * 2 : 0;
    const damageScore   = totalDmg  > 0 ? ((p.totalDamageDealtToChampions || 0) / totalDmg) * 100 : 0;
    const turretScore   = ((p.damageDealtToTurrets || 0) / dur)* 0.5;
    const kpScore       = (p.killParticipation ?? p.challenges?.killParticipation ?? 0) * 25;

    if (p.championName === 'Nocturne') ccScore /= 4;
